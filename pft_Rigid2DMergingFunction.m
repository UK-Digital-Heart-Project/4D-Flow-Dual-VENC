function pft_Rigid2DMergingFunction(LoVencMagnitudeSource, HiVencMagnitudeSource, LoVencPhaseSource, HiVencPhaseSource, MergedRoot)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Nominate some o/p folders
% 01. Motion-corrected Low-Venc modulus
MoCoLoVencMagnitudeTarget = fullfile(MergedRoot, 'OFFLINE - MOCO LOW-VENC MAGNITUDE - RIGID');
...
MoCoLoVencVelocityTarget = fullfile(MergedRoot, 'OFFLINE - MOCO LOW-VENC VELOCITY - RIGID');
...
MoCoFusedVelocityTarget = fullfile(MergedRoot, 'OFFLINE - MOCO FUSED VELOCITY - RIGID');
...
FilteredMoCoFusedVelocityTarget = fullfile(MergedRoot, 'OFFLINE - FILTERED MOCO FUSED VELOCITY - RIGID');
...
DiscrepancyTarget = fullfile(MergedRoot, 'OFFLINE - MOCO FUSED VELOCITY DISCREPANCY - RIGID');
...
TwiceCorrectedVelocityTarget = fullfile(MergedRoot, 'OFFLINE - TWICE-CORRECTED FUSED VELOCITY - RIGID');
...
FilteredTwiceCorrectedVelocityTarget = fullfile(MergedRoot, 'OFFLINE - FILTERED TWICE-CORRECTED FUSED VELOCITY - RIGID');
...
ResidualTarget = fullfile(MergedRoot, 'OFFLINE - TWICE-CORRECTED FUSED VELOCITY RESIDUAL - RIGID');
...
ScreenshotTarget = fullfile(MergedRoot, 'OFFLINE - SCREENSHOTS - RIGID');
...
TransformationTarget = fullfile(MergedRoot, 'OFFLINE - TRANSFORMATION MATRICES - RIGID');
...
% 10. Transformation matrices have also been saved already

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Write out a small summary file
fid = fopen(fullfile(MergedRoot, 'Summary - Rigid Co-Registration.txt'), 'wt');
